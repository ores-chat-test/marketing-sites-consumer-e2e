const MESSAGE_PATH = "/marketing-sites-consumer-e2e/mock-api/v1/public/messages";
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith((async () => {
    ...
    if (!ALLOWED_CONTEXTS.has(contextId) || !message || message.length > 4_000) {
      return jsonResponse({ error: "invalid_public_request" }, 422);
    }

    return jsonResponse({
      reply: `Deterministic test reply for ${contextId}: received “${message}”.`,
